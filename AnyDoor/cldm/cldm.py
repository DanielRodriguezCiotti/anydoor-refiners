...
class ControlledUnetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, **kwargs):
        # print("ControlledUnetModel forward for timestep", timesteps)
        # print("NormX",torch.norm(x))
        # print("NormControl",torch.norm(control))
        # print("NormContext",torch.norm(context))
        ...
        for i, module in enumerate(self.output_blocks):
            if only_mid_control or control_copy is None:
                h = torch.cat([h, hs.pop()], dim=1)
            else:
                h = torch.cat([h, hs.pop() + control_copy.pop()], dim=1)
            h = module(h, emb, context)
